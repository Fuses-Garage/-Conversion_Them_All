<?php
	if(isset($_SESSION["units"])){
		...
		for($i=0;$i<$size;$i++){//すべての単位について
			if($i!=0){//1つ目でなければ
				$b=htmlspecialchars("1".$_SESSION["units"][$i]["name"]."=".$_SESSION["units"][$i]["bef"].$_SESSION["units"][$i-1]["name"]);//前の単位のレートとまとめて記述
			}else{
				$b="";
			}
			echo"<tr><td>",htmlspecialchars($_SESSION["units"][$i]["name"]),"</td><td>",$b,"</td></tr>";
		}
		echo"</table>";
		echo'<hr>';//横棒で区切る
	}
