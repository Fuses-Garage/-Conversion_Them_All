<?php
	if(isset($_SESSION["units"])){
		...
		for($i=0;$i<$size;$i++){//すべての単位について
			if(preg_match("/^[0-9]+(\.[0-9]*)?$/",$_SESSION["units"][$i]["bef"]))$rate*=$_SESSION["units"][$i]["bef"];
			if(preg_match("/^[0-9]+(\.[0-9]*)?$/",$_REQUEST[$_SESSION["units"][$i]["name"]]))$summinunit+=$_REQUEST[$_SESSION["units"][$i]["name"]]*$rate;
